...
(*Iniciado dia 29/07, puxando módulo List *)
(*Autor: geremias corrêa*)

(*Aqui também se encontram as resoluções da lista de 5 de polimorfismo.
Havia o documento ListaPoly.v para realizá-lo, porém por problema de import ficou mais fácil fazer aqui
A resolução deles se encontram anexados ao final. Na linha 1076 (prox do fim)*)
(*Para achar um ctrl f com: "LISTA 5" a encontra*)

(* Suppress some annoying warnings from Coq: *)
Set Warnings "-notation-overridden,-parsing".
(*From LF Require Export Lists. (*Já que não consigo importar*)*)
...
(*Import manual advindo do Lists.v, Basics.v - tinha importado os notation do List tbm, mas ele vai importar dps:*)
Module NatList.
...
(*Obs: Aqui nil e const são construtores polimórficos, difernete de no Lists.
Devem ser passados neles um parâmetro que identifica o tipo da lista sendo construída*)
...
(*Nessa acima não declarando de forma clara qual o tipo, só que ele possuiu um qualquer*)
...
(*Aqui repare no {X} sendo o tipo*)
Arguments nil {X}. (*Aqui só passo o tipo*)
Arguments cons {X} _ _. (*Tem cabeça e cauda da lista tbm*)
Arguments repeat {X} x count. (* o valor x é numero a ser repetido e count a qtd de vezes*)
...
(*Sendo assim, não preciso declarar o tipo, ele inferi automaticamente nesses casos*)
...
(*Obs: descobri que dá pra fazer direto induction l.
    Só daí a IH fica padrão com IHl, por exemplo.
    Assim como quando é uso do rewrite com ->, pode optar pode deixar sem nada, significa o mesmo*)

(*Fazer esses exercícios de fixação - não conta na tarefa*)

Theorem app_nil_r : forall (X:Type), forall l:list X,
  l ++ [] = l.
Proof.
    intros X l. induction l as [ | h t IH]. (*ou induction l.*)
  - reflexivity.
  - simpl. rewrite -> IH. reflexivity.
 Qed.

(*Continuar a partir daqui para 03/08*)

Theorem app_assoc : forall A (l m n:list A),
  l ++ m ++ n = (l ++ m) ++ n.
Proof.
  intros A l m n. induction l as [ | h t IH]. (*Preciso do tipo A*)
  - reflexivity.
  - simpl. rewrite <- IH. reflexivity.
Qed.
...
Proof.
  intros X l1 l2. induction l1 as [ | h t IH]. (*Preciso do tipo X*)
  - simpl. reflexivity.
  - simpl. rewrite -> IH. reflexivity.
Qed.
...
Check prod. (*Tem tipo A, tipo B e constroi um tipo novo*)

(*Check (prod nat bool).*) (*É um tipo nat com bool, que é do tipo set daí (dado criado)*)

Check pair.

(*Check pair nat bool 2 true.*)
(*Check pair nat nat 2 2.*)

(*Os tipos x e y são implícitos e devem ser inferidos*)
Arguments pair {X} {Y} _ _.

(*Check pair 2 true.*) (*Agora posso apenas passar assim que ele pega o tipo*)
...
(*Mesmo que o map do haskell, seugndo prof - ? *)
...
(*Importei o do eqb =? etc para uso aqui e etc*)
...
(*Essa foi explicada em aula, no dia 03/08*)
...
(** For example, if we apply [filter] to the predicate [evenb]
    ...
    even members of [l]. *)
(*import para fazer funfar o test_filter1*)
...
Definition oddb (n:nat) : bool   :=   negb (evenb n). (*para aux na countodd abaixo*)
...
Compute (map (fun x => plus 3 x) [2;0;2]). (*ex aplicando função x = 3 + x*)
...
(*mesmo que fold_r do haskell - ideia da dobra*)
(*ex 1 + (2 + (3 + (4 + 0)))  *)
...
(*Se der, depois passar para a ListaPoly.v.*)

(*Lista 5 - questão 1:*) (*Isso?*)
(*Criar um map através do fold, sem recursão. Aqui usando fun (de função anonima) e tal*)
Definition fold_map {X Y: Type} (f: X -> Y) (l: list X) : list Y :=
  fold (fun x lista => f x :: lista) l nil.
...
(*As três abaixos são com uso do Tatics.v, não sei se precisa importar algo sla*)
(*Ou tentar resolver naquela ListaPoly.v*)
(*Lista 5 - questão 3*) (*Não entendi como fazer, usando plus_n_Sm ou double do induction talvez*)
