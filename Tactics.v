...
(*Importar o devido do Poly.v para conseguir executar isso aqu*)

Set Warnings "-notation-overridden,-parsing".
(*From LF Require Export Poly.*)

(*Como não é possível importar do Poly, importar o necessário abaixo:*)
...
(** Here, we could finish with "[rewrite -> eq2.  reflexivity.]" as we
  ...
  apply eq2.  Qed. (*Usamos o apply quando temos no contexto uma premissa que é exatamente a conclusão do que quero provar*)
...
Theorem silly_ex :
     ...
     evenb 4 = true.
Proof.
  intros eq1 eq2. apply eq2. Qed. (*eq1 é o forall..., oddb 3 = true é o eq2,*)
...
(** Here we cannot use [apply] directly, but we can use the [symmetry]
  ...
  symmetry. (*Ela invrte o lado da igualdade*)
...
(** If we simply tell Coq [apply trans_eq] at this point, it can
  ...
  apply trans_eq with (m:=[c;d]). (*forçando a fazer m substituir por [c;d]*)
...
Proof.
  intros n m H1.
  assert (H2: n = pred (S n)). { reflexivity. } (*forma rápida para criar uma hipótese e permitir usar ela em abaixo*)
...
Proof.
  intros n m o H.
  injection H. intros H1 H2. (*introduzi que é injetora com o injection. Sem uso do as preciso dar o intros*)
...
Proof.
  intros n m H.
  injection H as Hnm. rewrite Hnm. (*com o uso do as já faz o injection e o intros*)
  reflexivity. Qed.
...
(** We can proceed by case analysis on [n]. The first case is
  ...
  destruct n as [| n'] eqn:E. (*desmonta o n nas duas possibilidades: 0 ou sucessor de um n'*)
  - (* n = 0 *)
...
Theorem discriminate_ex1 : forall (n : nat),
  ...
  2 + 2 = 5.
Proof.
  intros n contra. discriminate contra. Qed. (*princípio da explosão, pois a premissa é falsa assim como a implicação*)
  (*usa o discriminate para isso (provar a contradiução), no caso*)
