...
(** Exercício 1 de revisão de COQ - nandb (not A ou not B)*)
...
(** Exercício 2. Porta and com 3 três entradas.*)
...
Definition inverteBit (b : bit) : bit :=
  match b with
  | B0 => B1
  | B1 => B0
  end.

Definition invNybble (b : nybble) : nybble :=
  match b with
  | bits b0 b1 b2 b3 => bits (inverteBit b0) (inverteBit b1) (inverteBit b2) (inverteBit b3)
  end.

Theorem inv_involutive: forall (b:bit), inverteBit (inverteBit b) = b.
Proof. intros b. destruct b.
 - simpl. reflexivity. 
 - simpl. reflexivity.
Qed.

Definition inv_involutive' (b:bit):inverteBit(inverteBit b) =b :=
  match b as b0 return (inverteBit (inverteBit b0) = b0) with
  | B0 => eq_refl
  | B1 => eq_refl
  end.
...
Theorem inv_nybble_involutive : forall (b:nybble), invNybble (invNybble b) = b.
Proof. intros a. destruct a. simpl. rewrite inv_involutive. rewrite inv_involutive. rewrite inv_involutive. rewrite inv_involutive.
reflexivity.
Qed.
...
(* Fatorial nada mais é que mult pelo predecessores do número*)
...
(** Usamos S que recebe um parâmetro e retorna o valor segunte menor que ele.*)
(** Ex 3!: 3 * 2 * 1 * 1*)
...
(*Lembrar que isso aqui uso no futuros em outros arquivos, preicso importar a mão*)
...
(*Ex 4 - verificar se n menor que m*)
Fixpoint ltb (n:nat) (m:nat) : bool := 
  match n with
  | 0 => 
    match m with
    | 0 => false
  ...
  | S nProx =>
    match m with
    | 0 => false
    | S mProx => ltb nProx mProx
    end
  end.
(* Parece que quando a função chama ela mesma, é fixpoint*)
...
Proof.
  ...
  (* rewrite the goal using the hypothesis: *)
  rewrite -> H.
  simpl. reflexivity.  Qed. (*Dando erro na reflividade, não consegue unificar m + o com m + m*)
  (*Testar dps com n + m = m + o.*)
...
Para todo n e m pertencente aos naturais, se m = n +1, então 
m*(n+1) == m*m. yeap. *)
...
(*Exercício 7 (com o professor): Para todos b c, tal que b e c são true e dado um c tbm true:*)
 Theorem andb_true_elim2 : forall b c : bool,
  andb b c = true -> c = true.
Proof.
  intros b c H. destruct c. reflexivity. destruct b. discriminate H. discriminate H. Qed.
 (* ou:
...
(* desconstruímos o c, temos duas posibilidades pro b, true = true e false = true. Por reflexividade provamos que true = true. Agora, pra provar false = true, precisamos de um destruct novamente no b e usamos o descriminate duas vezes pra provar que a hipótese é falsa*)

(** **** Exercise: 1 star, standard (zero_nbeq_plus_1)  *)
(* Exercício 8 (com professor)*)(*
