...
(*Só vou conseguir construir even, se esse natural for um número par*)
(*NEste caso aqui, se n é par, então sucessor do sucessor de n é par*)
...
Theorem ev_double : forall n,
  even (double n).
Proof.
  intro n. induction n as [| n' HI]. Admitted. (*ver dps*)
...
(*Diz que se tem um n que é par, então tem um valor k qlqr tq n é o dobro de k.*)
...
(*Cria 2 axiomas, 0 e 2 são pares, se n é par e m é par, então n+m é par*)
Inductive even' : nat -> Prop :=
| even'_0 : even' 0
| even'_2 : even' 2
...
(*Questão da lista de MFO - ver até aqui para quarta - FEITO*)
...
(*Do tatics para uso na função abaixo*)
...
(*três fixpoint abaixo importado de Poly.v*)
...
(*A concatenação dessa lista inteira irá ser gerada pela expressão fechada:  *)
(*(Para todo s tq s pertence a ss -> s = ~a*  *)
(*se ss = [a,b], então seria ab= ~(a|b)* *)
Lemma MStar' : forall T (ss : list (list T)) (re : reg_exp),
  (forall s, In s ss -> s =~ re) ->
  fold app ss [] =~ Star re.
Proof.
  intros  T ss re H. induction ss as [ | n t HI].
  - simpl. apply MStar0. (*qndo lista é vazia, provar que vazio faz parte do fechamento*)
  - simpl. apply MStarApp. (*Aqui a cabeça concatenada com *)
    + apply H. simpl. left. reflexivity.
    + apply H1. intros s Hs. apply H. simpl. right. apply Hs.
Qed.
...
Proof
  (* Ver print que tirei, é bem grande. *) Admitted.
(** [] *)
...
Proof.
  ...
  induction Hmatch
    as [| x'
        | s1 re1 s2 re2 Hmatch1 IH1 Hmatch2 IH2  (*s1= ?,re1 = expressõ regular 1, Hmatch é a evidencia, IH1 a hipótese relacionado a evidência*)
  ...
  - (* MChar *)
    apply Hin.
  - (*MApp *) simpl. rewrite In_app_iff in *.
