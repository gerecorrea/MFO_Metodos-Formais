...
(*Nesse arquivo está contido também a resolução da Lista 4 da disciplina*)
(*Pode ser encotnrado por ctrl f com "Lista 4 - questão x"*)
(*Tal que "x" é o número da questão, de 1 a 8*)
...
(*Obs: natprod recebe um produto de naturais, então p é um par (a,b) *)
Proof.
  intros p. destruct p as [a b]. reflexivity.
(** [] *)
...
(*Retorna a cabeça (head) de uma lista*)
...
Compute (hd 0 [1;2;3;4]). (*0 é um valor default de retorno*)

(*Retorna a cauda (tail) de uma lista*)
...
(*Retorna uma lista com elementos que não são zeros - fizemos em sala*)
...
Compute (nonzeros [1;0;2;0;3;0;5]).

Example test_nonzeros:
  nonzeros [0;1;0;2;3;0;0] = [1;2;3].
  (* FILL IN HERE *) Admitted.

(*Auxiliar para lista 4 - questão 2*)
(*Criei um em uma função, apesar de ter um com negando o evenb no Basics.v*)
...
(*Entro com um n, decremento 2 toda chamada, se chegar em 1 é ímpar, se for 2 (S 1) é par.*)

Example test_isodd:    isodd 1 = true.
Proof. simpl. reflexivity.  Qed.

(*Lista 4 - questão 2*)
(*Quero uma lista A que cria uma lista B só com ímpares*)
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with
  | [] => [] (*lista vazia*)
  | h::t => match isodd h with
            | true => h::(oddmembers t)
            | false => oddmembers t 
            end
  end.
  (*Se oddb que chama evenb retorna true, implica que é impar, portanto adiciono a lista*)
  (*Vi que também dá para fazer com if, mas fica paradigma imperativo daí:
  h::t => if (isodd h) then h::(oddmembers t) else oddmembers t    *)
Compute (oddmembers [1;2;3;4;5;6;7;8;9]).
...
Compute(countoddmembers [1;2;3;4;6;7;8;9]).
...
(*Alterna os valores de duas listas para uma terceira - fizemos sozinho*)
Fixpoint alternate (l1 l2 : natlist) : natlist :=
  match l1, l2 with
  | [], l2 => l2
  | l1, [] => l1
  | h1::t1, h2::t2 => h1::h2::(alternate t1 t2)
  (*Obs: em haskell poderia fazer h1::(alternativa l2 t1)*)
  end.
    
Compute(alternate [1;2;3] [4;5;6]).

(*Forma pela do professor*)
Fixpoint alternate' (l1 l2 : natlist) : natlist :=
  match l1 with
  | [] => l2
...
(*auxx - Lista 3, questão 1 - disponível no arquivo: Lista3-Q1-plus_comm.v*)
...
(** We've seen that proofs can make use of other theorems we've
  ...
  Search rev. (*Printa os teoremas definidos no arquivo e dos imports realizados*)
...
Theorem app_nil_r : forall l : natlist,
  l ++ [] = l. (*lista A concatenada a uma lista B vazia = lista A*)
Proof.
  intro p. induction p as [ | h t IH].
  - simpl. reflexivity.
  - simpl. rewrite -> IH. reflexivity. Qed.

(*Lista 4 - questão 5*)
(*Obs: rev reverte uma lista*)
Theorem rev_app_distr: forall l1 l2 : natlist,
  rev (l1 ++ l2) = rev l2 ++ rev l1.
Proof.
  intros p1 p2. induction p1 as [ | h t IH]. 
  - simpl. rewrite app_nil_r. reflexivity.
  - simpl. rewrite -> IH. rewrite -> app_assoc. reflexivity. Qed.

(*Lista 4 - questão 6*)
Theorem rev_involutive : forall l : natlist,
  rev (rev l) = l. (*O reverso do reverso de uma lista é ela mesma*)
Proof.
  intro p. induction p as [ | h t IH].
  - simpl. reflexivity.
   - simpl. rewrite -> rev_app_distr. rewrite IH. simpl. reflexivity. Qed.
(*Tava um pouco perdido, mas no que usou o rev_app_distr pra trocar a disposição ficou com caso perfeito pra
aplicar o proprio rev_involutive e então só simplificar.*)
...
Theorem app_assoc4 : forall l1 l2 l3 l4 : natlist,
  l1 ++ (l2 ++ (l3 ++ l4)) = ((l1 ++ l2) ++ l3) ++ l4. (*propriedade da associatividade*)
Proof.
  intros p1 p2 p3 p4.
  - rewrite app_assoc. rewrite app_assoc. reflexivity.
Qed.
(*Obs: é basicamente o app_assoc (que é pra 3) só que pra 4, então seu uso se torna essencial para a prova.*)
...
(*Resolvida com professor:*)
Proof. 
  intros p1 p2. induction p1 as [ | h t IH]. (*tem cabeça(h), cauda(t) e hip. indução*)
  + simpl. reflexivity. 
  + simpl. destruct h as [ | h1].  (*Com o destruct abriu as possibilidades do h ser zero e de ser outro número natural.*)
    - rewrite IH. reflexivity.  (*prova para caso 0*)
    - simpl. rewrite IH.  reflexivity. (*prova para caso outros valores naturais*)
Qed.
