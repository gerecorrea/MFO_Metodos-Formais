(** * Logic: Logic in Coq *)
(*Parte da lista 6 de MFO*)

(*A resolução dos exercícios está inserida na ordem inicial das questões no arquivo
Para encontrá-los, pesquisar pelo padrão: 
  "Lista 6 - Logic.v - questão x"           

Obs: as que não estavam inseridas aqui e foram cobradas, coloque abaixo das relacionadas a ela
como dist_exists_and abaixo da dist_exists_or, caso fosse as usar.
*)

(*Realização dos imports necessário do Tatics.v e predecessores:*)
(* ########### IMPORTS NECESSÁRIOS ########## *)
Module NatList.
...
Proof.
  ...
  - reflexivity. (*caso a*)
  - simpl. rewrite -> IHn. reflexivity. (*caso b*)
Qed.
...
(*Fazer ese rapidinho dps - usa lá pela linha 1400+*)
Theorem eqb_true : forall n m,
    n =? m = true -> n = m.
Proof. Admitted.
  (*intros n m. simpl. induction m.
  - simpl. destruct n.
    + reflexivity.
    + discriminate.
  - simpl. destruct n.
...
Example and_example' : 3 + 4 = 7 /\ 2 * 2 = 4.
(*Uma igualdade entre naturais retorna um prop*)
(*Ali no and-example entra dois props como parametros e retorna um prop (true)*)
Proof.
  apply and_intro.
  - (* 3 + 4 = 7 *) reflexivity.
  - (* 2 + 2 = 4 *) reflexivity.
...
Proof.
  intros n m H. split.
  - destruct n as [ | n'].
    + (* n = 0 e m qualquer*) reflexivity. (*como para m =0 e m = S m ' é só por reflexivity, não preciso dar destruct*)
    + (* n = S n'*) discriminate H. (*pq obviamente é falso*)
  - destruct n as [ | n'].
    + (* n = 0*) simpl in H. apply H.
...
Proof.
  (* WORKED IN CLASS *)
  intros n m H.
  destruct H as [Hn Hm]. (*como ambas são verdades, separo as duas*)
  rewrite Hn. rewrite Hm. (*com isso com o rewrite já que são 0, provo por reflexividade*)
  reflexivity.
Qed.
...
Lemma and_example2' :
  forall n m : nat, n = 0 /\ m = 0 -> n + m = 0.
Proof.
  intros n m [Hn Hm]. (*já no intros posso introduzir as hipoteses já sepradno elas*)
...
Lemma and_example2'' :
  forall n m : nat, n = 0 -> m = 0 -> n + m = 0.
Proof.
  intros n m Hn Hm. (*No caso do && nem preciso usar o o colchetes*)
  rewrite Hn. rewrite Hm.
  reflexivity.
Qed.
...
Proof.
  (* WORKED IN CLASS *)
  intros n m H.
  assert (H' : n = 0 /\ m = 0). (*introduz um teorema que n = m =0, e em {} ele prova esse teorema rapidamente*)
...
Proof.
  intros P Q [HP HQ].
  apply HQ. Qed.
  (*Tinha um atnerior aqui, esse abaixo, mas não roda.*)
  (*split. (*ver o que fatal aqui*)
  - (*left*) apply HQ.
  - (*right*) apply HP. Qed.*)
...
Proof.
  ...
  - (* Here, [n = 0] *)
    rewrite Hn. reflexivity.
  - (* Here, [m = 0] *)
    rewrite Hm. rewrite <- mult_n_O. (*importar o mult_n_0 do basics.v*)
    reflexivity.
Qed.
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q H1. unfold not. (*com unfold no not, temos a negação na implicação por False*)
  intros H2 H3. apply H2. apply H1. apply H3.  Qed.
(*Repare que nesses apply faço o termo da direita ser substituído/unificado pelo da esq, até chegar no H3 ali.*)

(** **** Exercise: 1 star, standard (not_both_true_and_false)  *)
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P). (*obs: A negação dessa contradição será verdadeira*)
Proof.
  intros P. unfold not. intro H. apply H. destruct H. apply H. Qed.
(*Essa fizemos em sala*)
...
Theorem not_true_is_false : forall b : bool,
  b <> true -> b = false. (* <> é diferente*)
Proof.
  (*Aqui usa do teorema que do falso provo qualquer coisa*)
  intros [] H.
  - (* b = true *)
    unfold not in H.
...
(* Como são definidos True e False na biblioteca:
Inductive True : Prop := I : True.
   Inductive False : Prop := I : .    *)
...
(* Lista 6 - Logic.v - questão 1*)
Theorem or_distributes_over_and : forall P Q R : Prop,
  P \/ (Q /\ R) <-> (P \/ Q) /\ (P \/ R).
Proof.
  intros P Q R. split. (*Separo em duas*)
  - intro H. inversion H. (*->*)
    + split. (*separo em duas novamente*)
      * left. apply H0. (*P v Q*)
      * left. apply H0. (*P v R*)
    + split. (*faço a metade da metade que falta*)
      * apply proj1 in H0. right. apply H0. (*Assim Q /\ R vira Q na hipótese 0, logo fica simples*)
      * apply proj2 in H0. right. apply H0. (*Assim Q /\ R  vira R*)
  - intro H.  inversion H. (*<- agora a outra metade inteira.*)
...
Proof.
  intros X P H. unfold not. intros [x' H1]. apply H1. apply H. Qed.
(*feito em sala com o professor*)
...
Proof.
  (*Temos uma equivalencia, logo tem que provar os dois lados da implicação*)
  intros X P Q. split. (*separa em esq e direita*)
  - intros [x' [HP | HQ]]. (*novas hipóteses para nova esquerda e direita*)
    + left. exists x'. apply HP.
    + right. exists x'. apply HQ.
  - intros [HP | HQ]. (*hipóteses para esquerda e direita*)
    + destruct HP as [x' HPx]. exists x'.
    left. apply HPx.
    + destruct HQ as [x' HQx]. exists x'. 
    right. apply HQx. Qed.

(* Lista 6 - Logic.v - questão 2*)
Theorem dist_exists_and : forall (X:Type) (P Q : X -> Prop),
  (exists x, P x /\ Q x) -> (exists x, P x) /\ (exists x, Q x).
Proof.
  intros X P Q [x' [HP HQ]].  split. (*Introduz o lado esquerdo da implicação como lados separados e em hipótse, já que a condição é de /\ e ambas precisam ser verdadeiras. Então divide*)
  - exists x'. apply HP. (* quantificamos x' tendo x : X na resolução e simplificamos pela hipótese HP*)
  - exists x'. apply HQ. (* mesma ideia, mas com HQ*)
  Qed.
...
Lemma In_map :
  forall (A B : Type) (f : A -> B) (l : list A) (x : A),
    In x l ->
    In (f x) (map f l).
  (*Se x está na lista, então x está na função que aplica o map nos elementos da lista*)
Proof.
  intros A B f l x.
...
(*é um dos exercícios da lista*)
(* Lista 6 - Logic.v - questão 3*)
Lemma In_map_iff :
...
Proof.
  ...
  - intros H. induction l as [| x1 l1 IHl1]. (*->*)
    + simpl in H. contradiction. (* l = [], simplifica e por contradição é provado *)
    + simpl in H. destruct H as [H1 | H2]. (* l = x' :: l', simplifica e abre *)
      * exists x1. split.
        -- apply H1. (*prova aplicando a hipótese H1*)
        -- simpl. left. reflexivity. (*simplifica, busco provar um lado (pois é \/) por reflexividade x1=x1*)
      * apply IHl1 in H2. destruct H2 as [x3 H3]. exists x3. split. 
        -- apply H3. (*basta aplicar hipótese H3*) (*apply proj1 in H2. apply H2.*)
        -- simpl. right. apply proj2 in H3. apply H3.
  -  intros H. induction l as [ | x1 l1 IHl1]. (* <- *)
    + simpl in H. destruct H as [x1 H]. apply proj2 in H. contradiction. (* l = [], provando por contradição*)
    + simpl. simpl in H. destruct H as [x2 H]. inversion H. destruct H1 as [H2 | H3]. (* l = x' :: l' *)
      * left. rewrite H2. apply H0.
      * right. apply IHl1. exists x2. split.
        -- apply H0.
        -- apply H3.
  Qed.
...
(*é um dos exercícios da lista*)
(* Lista 6 - Logic.v - questão 4*)
Lemma In_app_iff : forall A l l' (a:A),
  In a (l++l') <-> In a l \/ In a l'.
Proof.
  intros A l l1 x. split.
  - intros H0. induction l. (*->*)
    + simpl. simpl in H0. right. apply H0.
  ...
  - intros H0. induction l. (*<-*)
    + simpl. simpl in H0. destruct H0.
      * contradiction.
      * apply H.
    + simpl. simpl in H0. apply or_assoc in H0. destruct H0 as [H1 | [H2 | H3]].
      * left. apply H1. 
...
(* Lista 6 - Logic.v - questão 5*)
(*Segundo o professor, se basear na In*)
Fixpoint All {T : Type} (P : T -> Prop) (l : list T) : Prop :=
  (*Quero aqui construir uma lista em que todos os elementos P que satisfaçam a propriedade da proposição requisita*)
  match l with
  | [] => True
  | x' :: l' => P x' /\ All P l'
  end.
...
Proof.
  ...
  - intros H. induction l. (* -> *)
    ...
    + simpl. split.
      ...
      * apply IHl. intros x1 H1. apply H. simpl. right. apply H1.
  - intros H. induction l. (* <- *)
    + simpl. intros x1 H1. contradiction.
    + simpl. intros x1 H1. destruct H1 as [ |H2 H3].
      * simpl in H. apply proj1 in H. rewrite H0 in H. apply H. 
      * simpl in H. apply proj2 in H. apply IHl with x1 in H. apply H. apply H2. 
Qed.
